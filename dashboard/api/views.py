...
class CreateJobPostView(CreateAPIView):
    ...
    def perform_create(self, serializer):
     print("Is Authenticated:", self.request.user.is_authenticated)  # Should be True
     print("User:", self.request.user)  # Should show the username or authenticated user info
     print("Auth Header:", self.request.META.get("HTTP_AUTHORIZATION"))  # Check if the token is reaching the view

     if self.request.user.is_authenticated:
        serializer.save(poster=self.request.user)
     else:
        raise ValidationError({"status": "Error", "message": "User not authenticated"})
